...
export default function DocumentManagerPage() {
  ...
  const validateFiles = (files: FileList): boolean => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // File size limit - 750MB for all file types
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    console.log(
      `[EMTCHAT-PAGE-UPLOAD] Version: dd55c40-CONSOLE-PRESERVED-ENHANCED-DIAGNOSTICS - Initializing upload for ${files.length} files.`
    );
    ...
    // Pre-upload logging
    console.log(`[EMTCHAT-PAGE-UPLOAD] Starting direct-to-S3 upload flow`);
    console.log(`[EMTCHAT-PAGE-UPLOAD] Number of files: ${files.length}`);
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        console.log(`[EMTCHAT-PAGE-UPLOAD] Processing file ${i + 1}/${files.length}:`, fileInfo);

        try {
          // Step 1: Request pre-signed URL from backend
          console.log(`[EMTCHAT-PAGE-UPLOAD] Step 1: Requesting pre-signed URL for ${file.name}`);
          ...
          console.log(
            `[EMTCHAT-PAGE-UPLOAD] Received pre-signed URL for S3 key: ${s3Key}, expires in ${expiresIn}s`
          );
          ...
          // Check for reasonable file size (750MB limit as additional safety)
          if (file.size > 750 * 1024 * 1024) {
            throw new Error(`File too large: ${file.size} bytes (max 750MB)`);
          }
          ...
          console.log(`[EMTCHAT-PAGE-UPLOAD] Step 2: Uploading ${file.name} directly to S3`);
          ...
          console.log(
            `[EMTCHAT-PAGE-UPLOAD] S3 upload response status: ${s3UploadResponse.status}`
          );
          console.log(
          ...
          if (!s3UploadResponse.ok) {
            ...
            try {
              ...
              console.error(`[EMTCHAT-PAGE-UPLOAD] S3 error response body:`, errorText);
            } catch (textError) {
              console.error(`[EMTCHAT-PAGE-UPLOAD] Could not read S3 error response:`, textError);
            }
          ...
          console.log(`[EMTCHAT-PAGE-UPLOAD] Successfully uploaded ${file.name} to S3`);
          ...
          if (isSupportedImageType(file)) {
            console.log(`[EMTCHAT-PAGE-UPLOAD] Image detected, performing OCR for ${file.name}`);
            try {
              ...
              console.log(
                `[EMTCHAT-PAGE-UPLOAD] OCR complete. Confidence: ${ocrResult.confidence}%, Text length: ${extractedText.length}`
              );

              if (extractedText.length === 0) {
                console.warn(`[EMTCHAT-PAGE-UPLOAD] No text extracted from image ${file.name}`);
              }
            } catch (ocrError) {
              console.error(`[EMTCHAT-PAGE-UPLOAD] OCR failed for ${file.name}:`, ocrError);
              // Continue with upload even if OCR fails
            }
          }

          // Step 3: Notify backend for post-processing
          console.log(`[EMTCHAT-PAGE-UPLOAD] Step 3: Notifying backend for post-processing`);
          ...
          console.log(`[EMTCHAT-PAGE-UPLOAD] Backend processing initiated:`, processResult);
          ...
          console.error(`[EMTCHAT-PAGE-UPLOAD] Error uploading ${file.name}:`, fileError);
          errorCount++;
          processedFilesInfo.push({
            fileName: file.name,
            status: 'error',
      ...
      console.error(`[EMTCHAT-PAGE-UPLOAD] Unexpected error during upload:`, err);
      ...
      setUploadProgress(0);
      console.log(
        `[EMTCHAT-PAGE-UPLOAD] Upload process completed. Success: ${successCount}, Errors: ${errorCount}`
      );
    }
  };
    ...
    <ProtectedRoute>
      <div className="container mx-auto p-4 md:p-6 bg-background min-h-screen flex flex-col emtchat-bg">
        ...
        <Tabs defaultValue="documents" className="w-full flex-1 flex flex-col">
          <TabsList className="mb-4 w-full grid grid-cols-2 bg-neutral-200 dark:bg-neutral-700 p-1 rounded-lg">
            <TabsTrigger
              value="documents"
              className="data-[state=active]:bg-white data-[state=active]:text-neutral-900 dark:data-[state=active]:bg-neutral-800 dark:data-[state=active]:text-white rounded-md py-2 text-sm font-medium transition-all"
            >
              Documents
            </TabsTrigger>
            {user &&
          ...
          <TabsContent value="documents" className="flex-1 flex flex-col">
            ...
            <p className="text-muted-foreground mb-4 dark:text-neutral-400">
            ...
            <div className="card border border-neutral-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 shadow-md h-[calc(100vh-280px)] min-h-[400px]">
          ...
          {user &&
            (user.role === 'admin' ||
              ...
              <TabsContent value="settings" className="flex-1">
                <div className="card border border-neutral-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 shadow-md p-4 md:p-6">
                  {
                    user?.role === 'admin' ? (
