...
import { AdminTabSheet } from '@/components/admin/AdminTabSheet';
import {
  Select,
  SelectContent,
  SelectItem,
...
// --- START: Dynamic Import for Branded Word Viewer ---
const BrandedWordViewer = dynamic(
  () => import('@/components/BrandedWordViewer').then(mod => mod.default),
  {
    loading: () => (
      <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin text-amber-500" />
          <p className="mt-4 text-gray-600">Loading Word viewer...</p>
        </div>
      </div>
    ),
    ssr: false,
...
export default function AdminPage() {
  ...
  // --- State for Active Admin Tab (for mobile bottom sheet) ---
  const [activeAdminTab, setActiveAdminTab] = useState<string>('insights');
  ...
  // --- Document Viewer State (Admin) ---
  const [adminDocUrl, setAdminDocUrl] = useState<string | null>(null);
  const [adminViewingDocName, setAdminViewingDocName] = useState<string>('');
  const [adminViewerType, setAdminViewerType] = useState<'pdf' | 'word' | null>(null);
  const [adminDocId, setAdminDocId] = useState<string | null>(null);

  // --- State to control Document Viewer Dialog ---
  ...
  const handleTabChange = (value: string) => {
    ...
    setActiveAdminTab(value); // Update active tab for mobile bottom sheet
  ...
  const handleViewSystemKbDocument = async (docId: string, filename: string) => {
    ...
    const isWord = lowerFilename.endsWith('.docx') || lowerFilename.endsWith('.doc');
    const isPdf = lowerFilename.endsWith('.pdf');

    // Build streaming URL
    const apiUrl = getApiBaseUrl();
    const streamUrl = `${apiUrl}/api/system-kb/stream/${docId}`;

    console.log(`[Admin Viewer] File type detected: ${isWord ? 'word' : isPdf ? 'pdf' : 'other'}`);
    console.log(`[Admin Viewer] Stream URL: ${streamUrl}`);

    // For Word files, pass the URL directly to BrandedWordViewer
    // (it handles fetching and conversion internally)
    if (isWord) {
      setAdminViewerType('word');
      setAdminDocUrl(streamUrl);
      setIsAdminViewerOpen(true);
      return;
    }

    // For PDF files, fetch as blob and create object URL
    if (isPdf) {
      try {
        const response = await fetchWithAuth(`/api/system-kb/stream/${docId}`, { credentials: 'include' });

        console.log(
          `[Admin Viewer] Fetch completed. Status: ${response.status}, OK: ${response.ok}, Content-Type: ${response.headers.get('Content-Type')}`
        );

        if (!response.ok) {
          const errorText = await response.text();
          console.error(`[Admin Viewer] Fetch failed! Status: ${response.status}, Body: ${errorText}`);
        ...
        setAdminViewerType('pdf');
        setAdminDocUrl(objectUrl);
        setIsAdminViewerOpen(true);
      } catch (err: unknown) {
        ...
        setAdminViewerType(null);
        setIsAdminViewerOpen(false);
      }
      return;
    }

    // For other file types, show a toast message
    toast({
      title: 'Unsupported File Type',
      description: `Cannot preview "${filename}". Only PDF and Word documents are supported.`,
      variant: 'destructive',
    });
  };
  ...
  return (
    <ProtectedRoute>
      ...
      {!user || user.role !== 'admin' ? (
        ...
        <div className="flex flex-col h-screen bg-background emtchat-bg">
          {/* Header */}
          ...
          <div className="flex-1 overflow-hidden">
            ...
            >
              {/* Mobile: Bottom Sheet Tab Selector */}
              <div className="m-4 mb-0 flex-shrink-0">
                <AdminTabSheet
                  tabs={[
                    { id: 'insights', label: 'Insights', icon: <Lightbulb className="h-4 w-4" /> },
                    { id: 'system-kb', label: 'System KB', icon: <Database className="h-4 w-4" /> },
                    { id: 'users', label: 'Users', icon: <Users className="h-4 w-4" /> },
                    { id: 'usage', label: 'Usage', icon: <BarChart3 className="h-4 w-4" /> },
                    ...(user?.username === 'dev' ? [{ id: 'audit-logs', label: 'Audit Logs', icon: <ClipboardList className="h-4 w-4" /> }] : []),
                ...
                />
                {/* Desktop: Traditional Tab List */}
                <TabsList className="hidden md:flex w-full">
                  <TabsTrigger value="insights" data-help-id="admin-insights">Insights</TabsTrigger>
              ...
              {/* Scrollable Tab Content - with safe area for mobile browsers */}
            ...
            {/* Document Viewer - PDF or Word (Used for System KB documents) */}
            {isAdminViewerOpen && adminDocUrl && adminViewerType === 'pdf' && (
              <BrandedPdfViewer
                fileUrl={adminDocUrl}
                title={adminViewingDocName}
                documentId={adminDocId || undefined}
                documentType="system"
                onClose={() => {
                  setIsAdminViewerOpen(false);
                  // Revoke object URL when closing the viewer (only for blob URLs)
                  if (adminDocUrl && adminDocUrl.startsWith('blob:')) {
                    URL.revokeObjectURL(adminDocUrl);
                  }
                  setAdminDocUrl(null);
                  setAdminViewerType(null);
                }}
            ...
            {isAdminViewerOpen && adminDocUrl && adminViewerType === 'word' && (
              <BrandedWordViewer
                fileUrl={adminDocUrl}
                title={adminViewingDocName}
                documentId={adminDocId || undefined}
                ...
                onLoadError={(error: Error) => {
                  console.error('[Admin Viewer] Word load error:', error.message);
                  toast({
                    title: 'Error Loading Document',
