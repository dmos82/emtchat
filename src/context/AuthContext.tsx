...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const checkSession = useCallback(async (): Promise<void> => {
    ...
    // Check for token first - if none, skip API call entirely
    const token = typeof window !== 'undefined' ? localStorage.getItem('accessToken') : null;
    if (!token) {
      console.log('[AuthContext] checkSession: No token found, skipping verification');
      setUser(null);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // Add timeout for iOS compatibility - prevent hanging
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        console.error('[AuthContext] checkSession: Request timed out after 10s');
      } else {
        ...
        if (response.ok) {
          ...
          if (data.success && data.user) {
            ...
            if (data.token) {
              ...
              // Dispatch event for components to refetch data (like useUserSettings)
              window.dispatchEvent(new CustomEvent('auth:login'));
              console.log('[AuthContext][login] auth:login event dispatched');
            } else {
              console.warn(
