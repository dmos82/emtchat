...
export const IMBuddyList: React.FC<IMBuddyListProps> = ({ onClose }) => {
  ...
  // Find 1-on-1 conversations with unread messages that aren't from users in onlineUsers list
  // This ensures users can always find unread messages even if the sender isn't showing
  const unreadConversationsNotInList = conversations.filter((c) => {
    if (c.isGroup) return false; // Groups are handled separately
    if (c.unreadCount === 0) return false; // No unread messages
    // Check if this conversation's user is NOT in the onlineUsers list
    const isInOnlineList = onlineUsers.some((u) => u._id === c.otherParticipant._id);
    return !isInOnlineList;
  });
  ...
  const handleUserClick = (user: UserPresence) => {
    ...
    // Immediately mark conversation as read (optimistic UI update)
    // This clears the red badge instantly before API call completes
    if (existingConv && existingConv.unreadCount > 0) {
      markConversationAsRead(existingConv._id);
    }
  ...
  // Handle clicking on an unread 1-on-1 conversation (for users not in online list)
  const handleUnreadConversationClick = (conv: typeof conversations[0]) => {
    // Immediately mark conversation as read (optimistic UI update)
    if (conv.unreadCount > 0) {
      markConversationAsRead(conv._id);
    }

    // Open the chat window with this user
    openChatWindow(conv.otherParticipant, conv._id);
  };
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left group ${
          isSelected
            ...
            : hasUnread
            ? 'bg-yellow-50 dark:bg-yellow-900/20 ring-1 ring-yellow-300/50 dark:ring-yellow-500/30'
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className={`text-sm truncate ${hasUnread ? 'font-bold text-slate-900 dark:text-white' : 'font-medium text-slate-700 dark:text-slate-200'}`}>
              {user.username}
            </span>
            {/* DND badge */}
            {user.dndEnabled && (
  ...
  // Unread conversations section (for users not in the online list)
  // This ensures the user can always find and clear unread messages
  const UnreadConversationsSection: React.FC = () => {
    if (isGroupMode || unreadConversationsNotInList.length === 0) return null;
    ...
    return (
      <div className="mb-3">
        ...
        <div className="space-y-0.5">
          {unreadConversationsNotInList.map((conv) => (
            <button
              key={conv._id}
              onClick={() => handleUnreadConversationClick(conv)}
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[#2a2a2a] transition-colors group"
            >
              {/* User avatar */}
              <div className="relative flex-shrink-0">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center text-slate-800 text-sm font-semibold shadow-sm">
                  {conv.otherParticipant.username.charAt(0).toUpperCase()}
                </div>
                <div className="absolute -bottom-0.5 -right-0.5">
                  <StatusBadge status={conv.otherParticipant.status} />
                </div>
              </div>

              {/* Username */}
              <span className="flex-1 text-sm font-bold text-slate-900 dark:text-white truncate text-left">
    ...
    >
      ...
      <div className="overflow-y-auto p-2" style={{ maxHeight: '350px' }}>
        {isLoadingUsers ? (
          ...
          <>
            {/* Unread messages from users not in the online list - show first for visibility */}
            <UnreadConversationsSection />
            {/* Groups section - show existing group conversations */}
            <GroupsSection />
      ...
      <div className="px-4 py-2 bg-slate-50 dark:bg-[#1a1a1a] border-t border-slate-200 dark:border-[#404040]">
        {isGroupMode ? (
          ...
          <div className="flex items-center justify-center">
            {/* Refresh button - simplified footer */}
            <button
              onClick={() => refreshOnlineUsers()}
              className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-yellow-600 dark:hover:text-yellow-500 transition-colors"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
