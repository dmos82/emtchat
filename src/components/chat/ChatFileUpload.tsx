    ...
    (files: FileList): File[] => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!allowedTypes.includes(file.type) && !allowedExtensions.includes(fileExtension)) {
          errors.push(
            `${file.name}: Unsupported file type. Only PDF, Word, Excel, TXT, Markdown, and image files are allowed.`
          );
          continue;
        }

        // Check file size (750MB limit for all files)
    ...
    <div className={cn('space-y-4', className)}>
      ...
      >
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          PDF, Word, Excel, TXT, Markdown, images, audio & video • Max 750MB
        </p>
      </div>
