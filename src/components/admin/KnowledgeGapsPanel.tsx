...
const KnowledgeGapsPanel: React.FC<KnowledgeGapsPanelProps> = ({ onNewGapCountChange }) => {
  ...
  // Test query state (to show model response)
  const [isTestingQuery, setIsTestingQuery] = useState(false);
  const [testQueryResponse, setTestQueryResponse] = useState<string | null>(null);
  ...
  // Auto-refresh polling interval (30 seconds)
  ...
  // silent: true skips loading state for background polling (prevents UI flicker)
  const fetchGaps = useCallback(async (silent = false) => {
    if (!silent) {
      setIsLoading(true);
    }
    setError(null);
  ...
  // Auto-refresh polling when panel is expanded
  useEffect(() => {
    // Only poll when panel is visible (not collapsed)
    if (!contentCollapsed) {
      // Start polling with silent=true to prevent loading flicker
      pollIntervalRef.current = setInterval(() => {
        console.log('[KnowledgeGapsPanel] Auto-refreshing data...');
        fetchGaps(true);  // silent refresh
        fetchStats();
      }, POLL_INTERVAL_MS);
    }

    // Cleanup: stop polling when collapsed or unmounted
    return () => {
  ...
  }, [contentCollapsed, fetchGaps, fetchStats]);

  // Test query to see current model response
  const handleTestQuery = async () => {
    if (!selectedGap) return;

    setIsTestingQuery(true);
    setTestQueryResponse(null);
    try {
      const response = await fetchWithAuth('/api/chat/query', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: selectedGap.query,
          mode: 'hybrid',  // Use hybrid mode to search both KB and user docs
        }),
      });
      ...
      const data = await response.json();
      setTestQueryResponse(data.response || data.message || 'No response received');
    } catch (err: any) {
      console.error('[KnowledgeGapsPanel] Error testing query:', err);
      setTestQueryResponse(`Error: ${err.message}`);
    } finally {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {stats ? `${stats.total} total, ${stats.new} new` : 'Loading...'}
            {lastUpdated && !contentCollapsed && (
              <span className="ml-2">
              ...
              <div className="space-y-2">
                ...
                {testQueryResponse && (
                  <div className="p-3 bg-muted/50 rounded-md border max-h-48 overflow-y-auto">
                    <p className="text-sm whitespace-pre-wrap">{testQueryResponse}</p>
                  </div>
                )}
                {!testQueryResponse && !isTestingQuery && (
                  <p className="text-sm text-muted-foreground italic">
                    Click &quot;Test Query&quot; to see how the model currently responds to this question.
                  </p>
                )}
              </div>
