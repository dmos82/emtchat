...
export default function SystemKBUpload({ onUploadSuccess }: SystemKBUploadProps = {}) {
  ...
  const validateFiles = (files: FileList): boolean => {
    const allowedTypes = ['.pdf', '.txt', '.docx', '.doc', '.xlsx', '.xls', '.md', '.markdown'];
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!allowedTypes.includes(fileExtension)) {
        setUploadError(`Invalid file type: ${file.name}. Allowed: PDF, TXT, Word, Excel, Markdown.`);
        return false;
      }

      if (file.size > maxSize) {
        setUploadError(`File too large: ${file.name}. Maximum size is 750MB.`);
        return false;
      }
    }
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              PDF, TXT, Word, Excel, Markdown (max 750MB each)
            </p>
          </div>
        </div>
