...
export default function FileTreeView({ onDocumentSelect, isActive }: FileTreeViewProps) {
  ...
  // Internal document viewer state (popup modal)
  const [viewingDocument, setViewingDocument] = useState<{ id: string; filename: string } | null>(null);

  // Handle file click - opens popup viewer
  const handleFileClick = useCallback((node: FileNode) => {
    console.log('[FileTreeView] Opening document in popup:', node._id, node.name);
    setViewingDocument({ id: node._id, filename: node.name });
  }, []);
  ...
  const renderNode = (node: FileNode, level: number = 0): React.ReactNode => {
    ...
    return (
      <div key={node._id}>
        <div
          className={cn(
            "flex items-center gap-2 px-2 py-1.5 hover:bg-accent hover:text-gray-900 rounded-md cursor-pointer transition-colors",
            !isFolder && "hover:bg-blue-50 hover:text-gray-900"
          )}
          style={{ paddingLeft: `${level * 16 + 8}px` }}
          // Make files draggable so they can be dropped into chat
          draggable={!isFolder}
          onDragStart={(e) => {
            if (isFolder) return;
            e.dataTransfer.effectAllowed = 'copy';
            // Store document metadata for chat to pick up on drop
            e.dataTransfer.setData('application/x-emtchat-document', JSON.stringify({
              ...
              fileSize: node.size,
              sourceType: 'system', // System KB documents
            }));
          ...
          onClick={() => {
            if (isFolder) {
              ...
              // For files, open in popup viewer
